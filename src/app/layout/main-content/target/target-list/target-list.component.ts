      ...
      if (paramsType) {
        switch (paramsType) {
          ...
          case TargetListParamsType.target_name:
            ...
            return `targets/?filter{target_name.icontains}=${targetName}`;
          case TargetListParamsType.uniprot_name:
            const uniprotName = params.get('uniprotName');
            return `targets/?filter{uniprot_name.icontains}=${uniprotName}`;
          case TargetListParamsType.gene_name:
            const geneName = params.get('geneName');
            return `targets/?filter{gene_name.icontains}=${geneName}`;
